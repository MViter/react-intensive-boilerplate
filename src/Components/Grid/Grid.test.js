...
const result = mount(<Grid api = { api } appID = { appID } />);

describe('<Grid/> component tests: ', () => {
    test('Should render \'section\' element for each item', () => {
        expect(result.find('section').length).toBe(1);
    });

    test('setState in getNews() should return state as expected.', async () => {
        await result.instance().getNews('abc-news');
        expect(result.state().news).toEqual([response]);
    });

    test('setState in handleSearchInput() should return state as expected.', async () => {
